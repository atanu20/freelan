    ...
    <a class="dropdown-item" href="our_course.php">Our Courses</a>
      ...
      <div class="container qu">
          ...
          if(isset($_POST['exm_sub']))
          {
            ...
            $st_exam=$_SESSION['exm_st_time'];
            // $mib_exm=$st_exam-$end_exam;

            // $date1=date_create($end_exam);
            // $date2=date_create($st_exam);
            // $diff=date_diff($date1,$date2);
            // $val= $diff->format("i:s");
           $one="0000:00:00 00:"."$st_exam".":00";
           $two="0000:00:00 00:"."$end_exam";

            $time1 = new DateTime($one);
$time2 = new DateTime($two);
$timediff = $time1->diff($time2);
$three= $timediff->format(' %i:%s');
            ...
            if(!empty($_POST['checker']))
            {
                ...
                $attemp=$count-$notattemp;
                $persent= ( ($result/$count)*100 );
                $persentval=round($persent, 2);
                date_default_timezone_set('Asia/Kolkata');
                $da=date('Y-m-d');
                $re="insert into result (user,category,our_result,attempt,not_attempt,totalque,persentage,date,duration_exam) values('$el','$ex_t','$result','$attemp','$notattemp','$count','$persentval','$da','$three')";
mysqli_query($con,$re);
?>
<div class="p-3">

<h4>your result is -> <?php echo $result ?></h4>
<hr>
<h4>Your Attempt questions are -> <?php echo $attemp ?> </h4>
<hr>
<h4>Your Not Attempt questions are -> <?php echo $notattemp ?> </h4>
<hr>
<h4> Your persentage is -> <?php echo $persentval ?> % </h4>
